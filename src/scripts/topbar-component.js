...
(function () {
    ...
    class AppTopbar extends HTMLElement {
        ...
        bindStp() {
            var alertEl = this.querySelector('[data-global-stp-alert]');
            ...
            var sync = function (status) {
                var isDisabled = status !== 'enabled';
                alertEl.classList.toggle('hidden', !isDisabled);
            };
        ...
        render(title) {
            return '' +
                ...
                '      </div>' +
                '    </div>' +
                '    <div data-global-stp-alert class="hidden rounded-lg border border-blue-200 bg-blue-50 p-3 dark:border-blue-500/30 dark:bg-blue-500/10">' +
                ...
                '          <button type="button" class="inline-flex items-center rounded-md px-2 py-1 text-sm font-semibold text-gray-700 hover:bg-gray-900/10 dark:text-gray-300 dark:hover:bg-gray-900/10 cursor-pointer">Learn more</button>' +
                '        </div>' +
                '      </div>' +
                '    </div>' +
