...
(function () {
  ...
  function buildDetailRowHTML(entry, colCount) {
    var hasRichDetails =
      ...
      (entry.details.activityLog && entry.details.activityLog.length > 0);
    ...
    if (hasRichDetails) {
      ...
      function addInfoRow(label, value) {
        if (!value) return;
        infoRows +=
          '<div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-6 px-4 py-3 border-b border-gray-200 dark:border-white/10">' +
            '<div class="text-xs font-medium uppercase tracking-wide text-gray-500 dark:text-gray-400">' + label + '</div>' +
            '<div class="text-sm font-medium text-gray-900 dark:text-white">' + escapeHtml(value) + '</div>' +
          '</div>';
      }

      addInfoRow('Type', info.type);
      addInfoRow('Exchange ID', info.exchangeId);
      addInfoRow('Source Currency', info.sourceCurrency);
      addInfoRow('Target Currency', info.targetCurrency);
      addInfoRow('Exchange Rate', info.exchangeRate);
      addInfoRow('Source Amount', info.sourceAmount);
      addInfoRow('Target Amount', info.targetAmount);
      addInfoRow('Settlement Date', info.settlementDate);
      addInfoRow('Cardholder', info.cardholderName);
      addInfoRow('Card Number', info.cardNumber);
      addInfoRow('Expires', info.expires);
      addInfoRow('Account Holder', info.accountHolder);
      addInfoRow('Bank Name', info.bankName);
      addInfoRow('Routing Number', info.routingNumber);
      addInfoRow('Account Number', info.accountNumber);

      (entry.details.attachments || []).forEach(function (att) {
        attachmentsRows +=
          '<div class="inline-flex items-center rounded-md border border-gray-200 bg-gray-50 px-2 py-1 text-xs text-gray-700 dark:border-white/10 dark:bg-white/5 dark:text-gray-300">' +
            escapeHtml(att.name || '') +
          '</div>';
      });

      (entry.details.activityLog || []).forEach(function (log) {
        var label = [log.user, log.action].filter(Boolean).join(' ');
        var meta = [log.date, log.timeAgo].filter(Boolean).join(' • ');
        var comment = log.comment ? ('<div class="mt-1 text-xs text-gray-500 dark:text-gray-400">' + escapeHtml(log.comment) + '</div>') : '';
        activityRows +=
          '<div class="py-2 border-b border-gray-200 dark:border-white/10 last:border-0">' +
            '<div class="text-sm font-medium text-gray-900 dark:text-white">' + escapeHtml(label) + '</div>' +
            '<div class="text-xs text-gray-500 dark:text-gray-400">' + escapeHtml(meta) + '</div>' +
            comment +
          '</div>';
      });
      ...
      return (
        '<tr data-detail class="hidden">' +
          '<td colspan="' + colCount + '" class="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-white/10">' +
            '<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 p-4 sm:p-6">' +
              '<div class="lg:col-span-1">' +
                '<h4 class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Notes</h4>' +
                '<p class="mt-2 text-sm text-gray-700 dark:text-gray-300">' + escapeHtml(entry.details.notes || '-') + '</p>' +
              '</div>' +
              '<div class="lg:col-span-2 rounded-lg border border-gray-200 dark:border-white/10 overflow-hidden">' +
                '<div class="px-4 py-3 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-white/5">Payment Method Details</div>' +
                (infoRows || '<div class="px-4 py-3 text-sm text-gray-500 dark:text-gray-400">No payment details</div>') +
              '</div>' +
            '</div>' +
            '<div class="px-4 sm:px-6 pb-4 sm:pb-6">' +
              '<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">' +
                '<div class="rounded-lg border border-gray-200 dark:border-white/10 p-4">' +
                  ...
                  '<div class="mt-3 flex flex-wrap gap-2">' + (attachmentsRows || '<span class="text-sm text-gray-500 dark:text-gray-400">No attachments</span>') + '</div>' +
                '</div>' +
                '<div class="rounded-lg border border-gray-200 dark:border-white/10 p-4">' +
                  '<h4 class="text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Activity Log</h4>' +
                  '<div class="mt-2">' + (activityRows || '<span class="text-sm text-gray-500 dark:text-gray-400">No activity yet</span>') + '</div>' +
                '</div>' +
              '</div>' +
            '</div>' +
          '</td>' +
